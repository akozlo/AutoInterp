...
class PipelineUI:
    ...
    def get_failed_steps(self) -> List[str]:
        """Return display names of steps that failed."""
        return [s.display_name for s in self.steps.values() if s.status == "failed"]

    def pipeline_complete(self, result: Dict[str, Any]) -> None:
        """Called when the pipeline finishes (may include failed steps)."""
        ...
        if self.rich_enabled:
            ...
            if failed:
                ...
                self.console.print(
                    Panel(
                        f"[bold yellow]Pipeline Complete (with errors)[/bold yellow]{elapsed}\n"
                        f"[yellow]Failed steps: {fail_list}[/yellow]{report_line}",
                        border_style="yellow",
                        padding=(1, 2),
                    )
                )

...
async def streamlined_pipeline(framework: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Persist provider/model metadata for this run (used at publish time)
    ...
    run_meta_path = Path(project_dir) / "run_metadata.json"
    try:
    ...
    try:
        ...
        # Determine final status based on whether any steps failed
        failed_steps = pipeline_ui.get_failed_steps() if pipeline_ui else []
        final_status = "completed_with_errors" if failed_steps else "completed"
        ...
        if pipeline_ui:
            ...
            if failed_steps:
                print(f"[AUTOINTERP] WARNING: These steps failed: {', '.join(failed_steps)}")
            print("="*80 + "\n")

        # Mark run status in metadata
        # Re-derive path from path_resolver in case the project dir was renamed
        run_meta_path = Path(path_resolver.get_project_dir()) / "run_metadata.json"
        try:
            if run_meta_path.exists():
                with open(run_meta_path) as f:
                    run_meta = json.load(f)
            run_meta["status"] = final_status
            run_meta["finished"] = get_timestamp()
            if failed_steps:
                run_meta["failed_steps"] = failed_steps
            with open(run_meta_path, "w") as f:
                json.dump(run_meta, f, indent=2)
        except Exception:
            pass
        ...
        # Mark run as failed in metadata
        # Re-derive path from path_resolver in case the project dir was renamed
        run_meta_path = Path(path_resolver.get_project_dir()) / "run_metadata.json"
        try:
            if run_meta_path.exists():
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    pub_parser.add_argument("--author-first-name", default="",
                            help="Author first name (omit to use agent-name instead)")
    pub_parser.add_argument("--author-last-name", default="",
                            help="Author last name (omit to use agent-name instead)")
    pub_parser.add_argument("--author-institution", default="",
                            help="Author institution")
    pub_parser.add_argument("--author-email", default="",
                            help="Author email address")
    pub_parser.add_argument("--agent-name", default="",
                            help="Name of the AI agent (used as author when no explicit author is set)")
    pub_parser.add_argument("--research-orchestrator", default="",
                            help="Name of the human researcher who orchestrated the AI-assisted research")
    pub_parser.add_argument("--agent-description", default="",
                            help="Short description of the agent (displayed as italic text on Future Science)")
    ...
    # publish-all: batch publish every completed, unpublished project
    pub_all_parser = subparsers.add_parser("publish-all", help="Publish all completed, unpublished projects to Future Science")
    pub_all_parser.add_argument("--api-key", default=None,
                                help="Future Science API key (falls back to FUTURE_SCIENCE_API_KEY env var or config)")
    pub_all_parser.add_argument("--initiative-id", default="",
                                help="documentId of the target initiative on Future Science")
    pub_all_parser.add_argument("--contributor-email", default="",
                                help="Email address of the submitting contributor")
    pub_all_parser.add_argument("--dry-run", action="store_true",
                                help="Print what would be submitted without actually POSTing")
    pub_all_parser.set_defaults(command="publish-all")

    parser.set_defaults(command="run")
    return parser


def mark_projects_as_rejected() -> None:
    """Mark all REJECT_* projects in their metadata to prevent future publishing."""
    ...
    for project_dir in sorted(projects_dir.iterdir()):
        if not project_dir.is_dir() or not project_dir.name.startswith("REJECT_"):
            continue
        ...
        if not meta.get("rejected"):
            meta["rejected"] = True
            meta["rejected_date"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
            with open(meta_path, "w") as f:
                json.dump(meta, f, indent=2)
            rejected_count += 1
            print(f"Marked as rejected: {project_dir.name}")

    print(f"Marked {rejected_count} projects as rejected in metadata")


def _is_retriable_publish_error(exc: Exception) -> bool:
    """Return True if *exc* looks like a transient server/timeout error worth retrying."""
    import requests
    if isinstance(exc, (requests.exceptions.Timeout, requests.exceptions.ConnectionError)):
        return True
    err = str(exc).lower()
    return any(tok in err for tok in ("502", "503", "504", "timeout", "gateway"))
...
def run_publish_cmd(args: argparse.Namespace) -> None:
    ...
    author_first_name = args.author_first_name or fs_cfg.get("author_first_name", "")
    author_last_name = args.author_last_name or fs_cfg.get("author_last_name", "")
    author_institution = args.author_institution or fs_cfg.get("author_institution", "")
    author_email = args.author_email or fs_cfg.get("author_email", "")
    agent_name = args.agent_name or fs_cfg.get("agent_name", "") or "AutoInterp"
    research_orchestrator = args.research_orchestrator or fs_cfg.get("research_orchestrator", "")
    agent_description = args.agent_description or fs_cfg.get("agent_description", "")
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            result = publish_project(
                ...
                api_key=api_key or "",
                initiative_id=args.initiative_id or fs_cfg.get("initiative_id", ""),
            ...
            if _is_retriable_publish_error(e) and attempt < max_attempts:
                wait_time = 5 * attempt
                print(f"[AUTOINTERP] API timeout/error (attempt {attempt}/{max_attempts}), retrying in {wait_time}s...")
                time.sleep(wait_time)
            else:
                label = f"after {max_attempts} attempts" if attempt == max_attempts else ""
                print(f"[AUTOINTERP] Submission failed{' ' + label if label else ''}: {str(e)[:200]}")
                sys.exit(1)
...
def run_publish_all_cmd(args: argparse.Namespace) -> None:
    """Batch-publish all completed, unpublished projects to Future Science."""
    from AutoInterp.src.publish.future_science import publish_project, is_published
    from datetime import datetime

    projects_dir = PACKAGE_ROOT / "projects"
    if not projects_dir.is_dir():
        print("[AUTOINTERP] No projects/ directory found.")
        sys.exit(1)

    # Resolve settings: flag > env var > config
    fs_cfg: Dict[str, Any] = load_config().get("future_science", {})
    api_key = args.api_key or os.environ.get("FUTURE_SCIENCE_API_KEY", "") or fs_cfg.get("api_key", "")
    if not api_key and not args.dry_run:
        print("[AUTOINTERP] ERROR: No API key provided. Use --api-key, set FUTURE_SCIENCE_API_KEY, "
    ...
    author_first_name = fs_cfg.get("author_first_name", "")
    author_last_name = fs_cfg.get("author_last_name", "")
    author_institution = fs_cfg.get("author_institution", "")
    author_email = fs_cfg.get("author_email", "")
    agent_name = fs_cfg.get("agent_name", "") or "AutoInterp"
    research_orchestrator = fs_cfg.get("research_orchestrator", "")
    agent_description = fs_cfg.get("agent_description", "")

    # Scan all project directories
    candidates = sorted(d for d in projects_dir.iterdir() if d.is_dir())
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] [AUTOINTERP] Scanning {len(candidates)} projects...")
    sys.stdout.flush()
    ...
    # First pass: count eligible projects
    skipped_rejected = 0
    for project_dir in candidates:
        name = project_dir.name

        # Skip projects marked as REJECT_*
        ...
        if status == "completed" and not meta.get("published", False):
            eligible_count += 1

    print(f"[{datetime.now().strftime('%H:%M:%S')}] Found {eligible_count} eligible projects to publish")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Skipping {skipped_rejected} projects marked REJECT_*")
    sys.stdout.flush()

    # Second pass: process each project
    for idx, project_dir in enumerate(candidates, 1):
        name = project_dir.name
        ...
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] [{published + 1}/{eligible_count}] Publishing: {name}")
        sys.stdout.flush()
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                print(f"  -> Skipped (no report found)")
                skipped_no_report += 1
                break
            except Exception as e:
                if _is_retriable_publish_error(e) and attempt < max_attempts:
                    wait_time = 5 * attempt
                    print(f"  [Attempt {attempt}/{max_attempts}] Timeout/API error, retrying in {wait_time}s...")
                    sys.stdout.flush()
                    time.sleep(wait_time)
                else:
                    label = f" (after {max_attempts} attempts)" if attempt == max_attempts else ""
                    print(f"  -> FAILED{label}: {str(e)[:200]}")
                    failed += 1
                    break
        ...
        if succeeded:
            if args.dry_run:
                ...
                doc_id = result.get("documentId", "")
                print(f"  -> Success (documentId: {doc_id})")
            published += 1
            # Rate-limit submissions to avoid overwhelming Future Science API
            if not args.dry_run and published < eligible_count:
                print(f"  Waiting 15s before next submission...")
                sys.stdout.flush()
                time.sleep(15)
        sys.stdout.flush()

    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] [AUTOINTERP] publish-all complete!")
    print(f"  Published:            {published}")
    print(f"  Skipped (marked REJECT_*): {skipped_rejected}")
    print(f"  Skipped (not completed/errors): {skipped_status}")
    print(f"  Skipped (already published): {skipped_published}")
    print(f"  Skipped (no report):  {skipped_no_report}")
...
async def async_main(args: argparse.Namespace) -> None:
    ...
    """
    # Print ASCII art from title.txt in color #6bb9cf (muted steel cyan)
    ...
    if title_path.exists():
        try:
            ...
            # Color #6bb9cf in RGB is 107,185,207
            color_start = "\033[38;2;107;185;207m"
            color_end = "\033[0m"
    ...
    try:
        ...
        if True:
            ...
            logger.info(f"LLM interface re-initialized with provider: {updated_config.get('llm', {}).get('provider', 'unknown')} and model: {updated_config.get('llm', {}).get('model', 'unknown')}")
        ...
        # Use same muted steel cyan as ASCII art title (#6bb9cf = RGB 107,185,207)
        color_start = "\033[38;2;107;185;207m"
        color_end = "\033[0m"
        print(f"\n{color_start}" + "="*60)
...
def main(argv: Optional[List[str]] = None) -> None:
    """
    Primary entry point for running AutoInterp via `python main.py`.
    """
    ...
    if command == "publish-all":
        try:
            run_publish_all_cmd(args)
        except Exception as e:
            import traceback
            print(f"[AUTOINTERP] Publish-all failed: {e}")
            traceback.print_exc()
            sys.exit(1)
        return
    try:
